...
const SERVICE = /\b(?:massag|pilates|fisio|recovery|termoterapia|crioterapia|banheira|drenagem|shiatsu|lomi|thai|ayurv[eé]dica)\w*/i;
const DATE_PREFERENCE = /\b(?:hoje|amanh[aã]|segunda|ter[cç]a|quarta|quinta|sexta|s[aá]bado|domingo|manh[aã]|tarde|noite)\b/i;
...
export function detectSchedulingIntent(message: string): SchedulingIntent {
  ...
  const hasDatePreference = DATE_PREFERENCE.test(message);
  return {
...
export function buildSchedulingInstructions(message: string, bookingUrl = process.env.NEXTFIT_BOOKING_URL): string | undefined {
  ...
  const missing = [!intent.hasService ? "serviço" : undefined,
    !intent.hasDatePreference ? "dia ou período" : undefined].filter(Boolean).join(" e ");
...
- ${missing ? `Antes do link, faça uma única pergunta curta para descobrir: ${missing}.` : "O pedido já contém serviço e período; conduza diretamente ao link oficial em vez de oferecer confirmação manual."}
...
- ${missing ? `Faça uma única pergunta curta para descobrir: ${missing}.` : "O pedido já contém serviço e período."}
- Não invente horários e não diga que consultou ou reservou uma vaga.
- Quando já houver serviço e período, ofereça encaminhar a conversa para a equipe pelo handoff interno.
